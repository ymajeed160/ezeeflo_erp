...
import { Add, Edit, Delete, Visibility, Receipt, Undo, Cancel, PostAdd } from '@mui/icons-material';
...
const CashPaymentVouchers = () => {
  ...
  const handleSave = async () => {
    ...
    if (form.lines.some(l => !l.accountId || parseFloat(l.amount) <= 0)) { setError('Please fill all lines with account and amount'); return; }
    ...
    try {
      ...
      if (selected) {
        await cpvApi.update(selected.id, payload);
      } else {
        await cpvApi.create(payload);
      }
      setDialog(false);
      loadVouchers();
    } catch (err) {
  ...
  const handlePost = async (id) => {
    try { await cpvApi.post(id); loadVouchers(); }
    catch (err) { alert(err.response?.data?.message || 'Post failed'); }
  };

  const handleReverse = async (id) => {
    if (!window.confirm('Reverse this CPV? This creates an opposite journal entry.')) return;
    try { await cpvApi.reverse(id); loadVouchers(); }
  ...
  return (
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          <Typography variant="h4" fontWeight={700}>Cash Payment Vouchers</Typography>
          <Typography variant="body1" color="text.secondary">Record cash payments for expenses</Typography>
        </Box>
      ...
      {loading ? <CircularProgress /> : (
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {vouchers.length === 0 && (
                <TableRow><TableCell colSpan={7} align="center">No CPVs found</TableCell></TableRow>
              )}
              {vouchers.map(v => (
        ...
        <DialogContent>
          ...
          <Box sx={{ mt: 2, p: 2, bgcolor: 'grey.50', borderRadius: 2 }}>
            ...
            <Box display="flex" justifyContent="space-between" mt={1} pt={1} borderTop="1px solid" borderColor="divider">
              ...
              <Typography fontWeight={700} color="primary" fontSize="1.1rem">{formatCurrency(t.total)}</Typography>
            </Box>
          </Box>
        </DialogContent>
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleSave} disabled={saving}>
            {saving ? 'Saving...' : selected ? 'Update' : 'Save CPV'}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={viewDialog} onClose={() => setViewDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle>CPV Details - {selected?.voucherNumber}</DialogTitle>
        <DialogContent>
          {selected && (
            <Grid container spacing={2}>
              ...
              <Grid item xs={6} sm={3}><Typography variant="caption">Payee</Typography><Typography>{selected.payeeName || '-'} ({selected.payeeType})</Typography></Grid>
...
export default CashPaymentVouchers;
