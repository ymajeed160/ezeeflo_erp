# Clean build script for EzeeFlo Loyalty Tezhost deployment
$ErrorActionPreference = "Stop"

$deployDir = "c:\Yasir\ERPMultiTenant\ERPMTSuite\deploy-tezhost\loyalty_nodejs_app"
$srcBackend = "c:\Yasir\ERPMultiTenant\ERPMTSuite\ezeeflo_loyalty\loyalty_backend"
$srcFrontendBuild = "c:\Yasir\ERPMultiTenant\ERPMTSuite\ezeeflo_loyalty\loyalty_frontend\build"
$zipPath = "c:\Yasir\ERPMultiTenant\ERPMTSuite\deploy-tezhost\loyalty_nodejs_app.zip"
...
$excludeDirs = @('node_modules', 'logs', 'uploads', 'tests', '.env', '.env.local')
...
$checks = @(
    ...
    @{Path="$deployDir\config\swagger.js"; Label="config/swagger.js"},
    @{Path="$deployDir\models"; Label="models/"},
    @{Path="$deployDir\seeders"; Label="seeders/"},
...
Write-Host "Zip: loyalty_nodejs_app.zip ($sizeMB MB, $totalEntries entries)"
Write-Host ""
Write-Host "Deploy structure:"
Write-Host "  root = backend files (server.js, app.js, controllers/, models/, ...)"
Write-Host "  front-end/build/ = React Loyalty application"
