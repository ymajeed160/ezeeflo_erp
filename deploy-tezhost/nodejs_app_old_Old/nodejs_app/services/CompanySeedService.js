...
class CompanySeedService {
  ...
  async seedAll(tenantId, userId) {
    ...
    try {
      ...
      await this._seedWalkInCustomer(tenantId, userId);
  ...
  async _seedPermissions(tenantId, userId) {
    const modules = {
      ...
      audit: ['view', 'export'],
      pos: ['view', 'open_session', 'create_sale', 'hold_sale', 'cancel_sale', 'return', 'discount', 'price_override', 'open_drawer', 'cash_in', 'cash_out', 'close_session', 'view_reports', 'manage_terminals', 'manage_settings', 'end_of_day'],
    };
  ...
  /**
   * Create a default Walk-In Customer for POS
   */
  async _seedWalkInCustomer(tenantId, userId) {
    const existing = await require('../models').Customer.findOne({
      where: { tenantId, code: 'WALK-IN' },
    });
    if (!existing) {
      // Find an asset account for AR
      const arAccount = await require('../models').Account.findOne({
      ...
      logger.info('  ✓ Created Walk-In Customer');
    }
  }
}
