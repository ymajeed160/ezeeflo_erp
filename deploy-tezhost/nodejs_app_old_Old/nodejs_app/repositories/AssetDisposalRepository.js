...
const { AssetDisposal, Asset, User } = require('../models');
const { Op } = require('sequelize');

class AssetDisposalRepository extends BaseRepository {
  constructor() { super(AssetDisposal); }

  async findByNumber(number, tenantId) {
    return await this.model.findOne({ where: { disposalNumber: number, tenantId } });
  }
  ...
  async findById(id, tenantId) {
    return await this.model.findOne({
      where: { id, tenantId },
      include: [
        { model: Asset, as: 'asset', attributes: ['id', 'assetCode', 'assetName', 'purchaseCost', 'accumulatedDepreciation', 'currentBookValue', 'status'], required: false },
  ...
  async findAndCountAll(tenantId, { page = 1, limit = 20, filters = {}, order = [['createdAt', 'DESC']], search = '' } = {}) {
    ...
    if (search) where[Op.or] = [{ disposalNumber: { [Op.like]: `%${search}%` } }];
  ...
  async getNextDisposalNumber(tenantId) {
    ...
    const num = parseInt(last.disposalNumber.replace('DSP-', ''), 10) + 1;
