...
3. Use that 16-character App Password in the Password field (not your regular Gmail password)`;
  ...
  /**
   * Send a custom email with full mailOptions control.
   * @param {Object} settings - Email settings from DB
   * @param {Object} mailOptions - { from, to, subject, html, attachments? }
   * @returns {Promise<Object>} { success, message }
   */
  static async sendCustomEmail(settings, mailOptions) {
    const transporter = this.createTransporter(settings);
    try {
      const info = await transporter.sendMail(mailOptions);
      logger.info(`Custom email sent to ${mailOptions.to}: ${info.messageId}`);
      return { success: true, message: `Email sent to ${mailOptions.to}` };
    } catch (error) {
      logger.error('Custom email failed:', { error: error.message, to: mailOptions.to });
      throw new Error(error.message);
    }
  }
}
