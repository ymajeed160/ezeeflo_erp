  ...
  async getAlreadyReceivedQty(tenantId, purchaseOrderId, itemId, excludeGoodsReceiptId = null) {
    ...
    const whereClause = {
      goodsReceiptId: { [Op.in]: sequelize.literal(
        `(SELECT id FROM \`${GoodsReceipt.getTableName()}\` WHERE \`purchase_order_id\` = '${purchaseOrderId}' AND \`tenant_id\` = '${tenantId}' AND \`status\` = 'received' AND \`deleted_at\` IS NULL${excludeGoodsReceiptId ? ` AND \`id\` != '${excludeGoodsReceiptId}'` : ''})`
      ) },
      itemId,
    };
