-- ============================================================================
-- EzeeFlo HR & Payroll — Stored Procedures for Reports
...
BEGIN
  SELECT e.employee_code, e.first_name, e.last_name, e.gender, e.nationality,
         e.work_email, e.mobile_number, e.joining_date, e.contract_end_date,
         d.name AS department, des.name AS designation, b.name AS branch,
         e.basic_salary, e.total_salary, e.status
  FROM employees e
  LEFT JOIN departments d ON d.id = e.department_id AND d.deleted_at IS NULL
  LEFT JOIN designations des ON des.id = e.designation_id AND des.deleted_at IS NULL
  ...
  WHERE e.tenant_id = p_tenant_id AND e.deleted_at IS NULL
    ...
    AND (p_department_id IS NULL OR p_department_id = '' OR e.department_id = p_department_id)
...
BEGIN
  SELECT e.employee_code, CONCAT(e.first_name, ' ', e.last_name) AS employee_name,
         ...
         pd.basic_salary, pd.allowances, pd.deductions, pd.overtime_pay,
         pd.loan_deduction, pd.gross_pay, pd.net_pay, pd.employer_contributions,
         pd.working_days, pd.paid_days, pd.absent_days
  FROM payroll_details pd
  JOIN employees e ON e.id = pd.employee_id AND e.deleted_at IS NULL
  LEFT JOIN departments d ON d.id = e.department_id AND d.deleted_at IS NULL
  LEFT JOIN designations des ON des.id = e.designation_id AND des.deleted_at IS NULL
  WHERE pd.tenant_id = p_tenant_id AND pd.deleted_at IS NULL
    AND pd.payroll_run_id = p_payroll_run_id
  ORDER BY e.employee_code;
END //
...
BEGIN
  SELECT e.employee_code, CONCAT(e.first_name, ' ', e.last_name) AS employee_name,
         ...
         lt.code AS leave_code, lt.name AS leave_type, lt.leave_category,
         lb.opening_balance, lb.accrued_days, lb.used_days, lb.pending_days,
         lb.available_balance, lb.carry_forward_days
  ...
  LEFT JOIN departments d ON d.id = e.department_id AND d.deleted_at IS NULL
  WHERE lb.tenant_id = p_tenant_id AND lb.deleted_at IS NULL
    AND lb.year = p_year
    AND (p_employee_id IS NULL OR p_employee_id = '' OR lb.employee_id = p_employee_id)
  ORDER BY e.employee_code, lt.leave_category;
END //
...
BEGIN
  ...
  WHERE oe.tenant_id = p_tenant_id AND oe.deleted_at IS NULL
    AND oe.overtime_date BETWEEN p_date_from AND p_date_to
...
BEGIN
  SELECT d.name AS department, d.code AS department_code,
         ...
         SUM(e.total_salary) AS total_salary_cost
  FROM departments d
...
BEGIN
  SELECT 
    ...
    SUM(CASE WHEN e.status IN ('Terminated', 'Resigned', 'Retired') THEN 1 ELSE 0 END) AS separated_count,
    COUNT(e.id) AS total_headcount,
    AVG(e.total_salary) AS avg_salary,
    SUM(e.total_salary) AS total_salary_budget
  FROM employees e
  WHERE e.tenant_id = p_tenant_id AND e.deleted_at IS NULL
    AND (p_as_of_date IS NULL OR e.joining_date <= p_as_of_date);
END //
...
BEGIN
  SELECT e.employee_code, CONCAT(e.first_name, ' ', e.last_name) AS employee_name,
         ec.calculation_date, ec.years_of_service, ec.basic_salary,
         ec.termination_type, ec.daily_wage, ec.first_5_years_amount,
         ec.after_5_years_amount, ec.total_eosb_amount, ec.max_cap_amount
  ...
  WHERE ec.tenant_id = p_tenant_id AND ec.deleted_at IS NULL
    AND ec.calculation_date BETWEEN p_date_from AND p_date_to
...
BEGIN
  SELECT e.employee_code, CONCAT(e.first_name, ' ', e.last_name) AS employee_name,
         el.loan_number, el.loan_type, el.principal_amount, el.monthly_installment,
         el.total_installments, el.paid_installments, el.remaining_amount,
         el.start_date, el.end_date, el.status
  FROM employee_loans el
  JOIN employees e ON e.id = el.employee_id AND e.deleted_at IS NULL
  WHERE el.tenant_id = p_tenant_id AND el.deleted_at IS NULL
...
BEGIN
  SELECT e.employee_code, CONCAT(e.first_name, ' ', e.last_name) AS employee_name,
         pa.appraisal_date, pa.period_from, pa.period_to, pa.overall_rating,
         pa.strengths, pa.improvements, pa.status,
         CONCAT(apr.first_name, ' ', apr.last_name) AS appraiser_name
  FROM performance_appraisals pa
  JOIN employees e ON e.id = pa.employee_id AND e.deleted_at IS NULL
  LEFT JOIN employees apr ON apr.id = pa.appraiser_id AND apr.deleted_at IS NULL
  WHERE pa.tenant_id = p_tenant_id AND pa.deleted_at IS NULL
    AND (p_employee_id IS NULL OR p_employee_id = '' OR pa.employee_id = p_employee_id)
...
BEGIN
  SELECT tc.code AS course_code, tc.name AS course_name, tc.category,
         ts.session_name, ts.start_date, ts.end_date, ts.trainer_name, ts.status AS session_status,
         COUNT(ta.id) AS total_enrolled,
         SUM(CASE WHEN ta.attendance_status = 'Attended' THEN 1 ELSE 0 END) AS attended_count,
         SUM(CASE WHEN ta.attendance_status = 'Completed' THEN 1 ELSE 0 END) AS completed_count,
         AVG(ta.score) AS avg_score
  FROM training_courses tc
  JOIN training_sessions ts ON ts.course_id = tc.id AND ts.deleted_at IS NULL
  LEFT JOIN training_attendees ta ON ta.session_id = ts.id AND ta.deleted_at IS NULL
  WHERE tc.tenant_id = p_tenant_id AND tc.deleted_at IS NULL
    AND (p_course_id IS NULL OR p_course_id = '' OR tc.id = p_course_id)
  GROUP BY tc.id, tc.code, tc.name, tc.category, ts.id, ts.session_name, ts.start_date, ts.end_date, ts.trainer_name, ts.status
