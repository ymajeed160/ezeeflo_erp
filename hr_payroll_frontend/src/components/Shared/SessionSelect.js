...
export default function SessionSelect({ value, onChange, label = 'Session', required = false, size = 'small', fullWidth = true }) {
  ...
  const fetch = useCallback(async (search = '') => {
    ...
    if (cache.length > 0 && now - lastFetch < 60000) {
      const filtered = search
        ? cache.filter(i => i.sessionName?.toLowerCase().includes(search.toLowerCase()))
        : [...cache];
      setItems(filtered);
      return;
    }
    setLoading(true);
    try {
      const r = await hrApi.get('/training-sessions', { params: { limit: 200, search } });
  ...
  return (
    <Autocomplete size={size} fullWidth={fullWidth} options={opts} loading={loading} value={sel} filterOptions={(x) => x}
      ...
      getOptionLabel={(i) => `${i.sessionName || ''} (${i.status || 'Planned'})`}
      ...
      renderOption={(p, o) => (<li {...p} key={o.id}><span style={{ fontWeight: 500 }}>{o.sessionName}</span>{o.course?.name && <span style={{ marginLeft: 8, color: '#888', fontSize: 12 }}>{o.course.name}</span>}<span style={{ marginLeft: 8, color: '#1976d2', fontSize: 11 }}>({o.status})</span><span style={{ marginLeft: 8, color: '#999', fontSize: 11 }}>{o.startDate}</span></li>)}
    />
  );
}
