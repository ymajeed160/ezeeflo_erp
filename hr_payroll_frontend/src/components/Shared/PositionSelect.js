...
export default function PositionSelect({ value, onChange, label = 'Position', required = false, size = 'small', fullWidth = true }) {
  ...
  const fetch = useCallback(async (search = '') => {
    ...
    if (cache.length > 0 && now - lastFetch < 60000) {
      const filtered = search
        ? cache.filter(i => i.title?.toLowerCase().includes(search.toLowerCase()) || i.positionCode?.toLowerCase().includes(search.toLowerCase()))
        : [...cache];
      setItems(filtered);
      return;
    }
    setLoading(true);
    try {
      const r = await hrApi.get('/job-positions', { params: { limit: 200, search } });
  ...
  return (
    <Autocomplete size={size} fullWidth={fullWidth} options={opts} loading={loading} value={sel} filterOptions={(x) => x}
      ...
      getOptionLabel={(i) => `${i.title || ''} (${i.positionCode || 'N/A'})${i.status ? ' — ' + i.status : ''}`}
      ...
      renderOption={(p, o) => (<li {...p} key={o.id}><span style={{ fontWeight: 500 }}>{o.title}</span><span style={{ marginLeft: 8, color: '#888', fontSize: 12 }}>{o.positionCode}</span>{o.status && <span style={{ marginLeft: 8, color: '#1976d2', fontSize: 11 }}>({o.status})</span>}{o.department?.name && <span style={{ marginLeft: 8, color: '#999', fontSize: 11 }}>{o.department.name}</span>}</li>)}
    />
  );
}
