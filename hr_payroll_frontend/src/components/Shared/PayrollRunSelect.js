...
export default function PayrollRunSelect({ value, onChange, label = 'Payroll Run', required = false, size = 'small', fullWidth = true }) {
  ...
  const fetch = useCallback(async (search = '') => {
    ...
    if (cache.length > 0 && now - lastFetch < 60000) {
      const filtered = search
        ? cache.filter(i => i.runNumber?.toLowerCase().includes(search.toLowerCase()) || i.period?.periodName?.toLowerCase().includes(search.toLowerCase()))
        : [...cache];
      setItems(filtered);
      return;
    }
    setLoading(true);
    try {
      const r = await hrApi.get('/payroll-runs', { params: { limit: 200, search } });
  ...
  return (
    <Autocomplete size={size} fullWidth={fullWidth} options={opts} loading={loading} value={sel} filterOptions={(x) => x}
      ...
      getOptionLabel={(i) => `${i.runNumber || '#' + (i.id || '').slice(0, 8)} — ${i.period?.periodName || ''} (${i.runDate || ''})`}
      ...
      renderOption={(p, o) => (<li {...p} key={o.id}>
        <span style={{ fontWeight: 500 }}>{o.runNumber || '#' + (o.id || '').slice(0, 8)}</span>
        <span style={{ marginLeft: 8, color: '#888', fontSize: 12 }}>{o.period?.periodName || ''}</span>
        <span style={{ marginLeft: 8, color: '#1976d2', fontSize: 11 }}>{o.runDate}</span>
      </li>)}
    />
  );
}
