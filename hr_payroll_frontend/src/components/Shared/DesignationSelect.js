...
export default function DesignationSelect({ value, onChange, label = 'Designation', required = false, size = 'small', fullWidth = true }) {
  ...
  const fetch = useCallback(async (search = '') => {
    ...
    setLoading(true);
    try {
      const r = await hrApi.get('/designations', { params: { limit: 200, search } });
  ...
  return (
    <Autocomplete size={size} fullWidth={fullWidth} options={opts} loading={loading} value={sel} filterOptions={(x) => x}
      ...
      renderOption={(p, o) => (<li {...p} key={o.id}><span style={{ fontWeight: 500 }}>{o.name}</span>{o.code && <span style={{ marginLeft: 8, color: '#888', fontSize: 12 }}>({o.code})</span>}</li>)}
    />
  );
}
