...
export default function DepartmentSelect({ value, onChange, label = 'Department', required = false, size = 'small', fullWidth = true }) {
  ...
  const fetch = useCallback(async (search = '') => {
    ...
    setLoading(true);
    try {
      const r = await hrApi.get('/departments', { params: { limit: 200, search } });
