...
export default function CourseSelect({ value, onChange, label = 'Course', required = false, size = 'small', fullWidth = true }) {
  ...
  const fetch = useCallback(async (search = '') => {
    ...
    setLoading(true);
    try {
      const r = await hrApi.get('/training-courses', { params: { limit: 200, search } });
  ...
  return (
    <Autocomplete size={size} fullWidth={fullWidth} options={opts} loading={loading} value={sel} filterOptions={(x) => x}
      ...
      getOptionLabel={(i) => `${i.name || ''} (${i.code || 'N/A'})${i.category ? ' — ' + i.category : ''}`}
      ...
      renderOption={(p, o) => (<li {...p} key={o.id}><span style={{ fontWeight: 500 }}>{o.name}</span><span style={{ marginLeft: 8, color: '#888', fontSize: 12 }}>{o.code}</span>{o.category && <span style={{ marginLeft: 8, color: '#1976d2', fontSize: 11 }}>({o.category})</span>}</li>)}
    />
  );
}
