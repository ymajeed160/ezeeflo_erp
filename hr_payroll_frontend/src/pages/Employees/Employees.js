...
import {
  ...
  TablePagination, CircularProgress, Alert, InputAdornment,
} from '@mui/material';
import {
...
import { apiSuccess, apiError, showSuccess, showError } from '../../utils/toast';
...
const Employees = () => {
  ...
  const { list, pagination, loading, saving, error } = useSelector((state) => state.employees);
  ...
  }, [dispatch, page, rowsPerPage, search, statusFilter, departmentFilter]);
  ...
  const handleDelete = async () => {
    ...
    if (deleteEmployee.fulfilled.match(result)) {
      ...
      showError(result.payload || 'Failed to delete employee');
    }
  };
  ...
  const handleCreateSubmit = async (data) => {
    ...
    if (createEmployee.fulfilled.match(result)) {
      ...
      showError(result.payload || 'Failed to create employee');
      return false;
    }
  };

  const handleEditSubmit = async (data) => {
    const result = await dispatch(updateEmployee({ id: selectedId, data }));
    if (updateEmployee.fulfilled.match(result)) {
      ...
      showError(result.payload || 'Failed to update employee');
      return false;
    }
  };
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        <Box>
          ...
          <Typography variant="body1" color="text.secondary">
            Manage employee profiles, contracts, and documents
          </Typography>
        </Box>
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} md={5}>
              <TextField
                fullWidth
                size="small"
                placeholder="Search by name, code, email, passport..."
                value={search}
            ...
            <Grid item xs={6} md={3}>
              <Button fullWidth variant="outlined" startIcon={<FilterIcon />} onClick={loadData}>
                Apply Filters
              </Button>
            </Grid>
          </Grid>
                  ...
                  {list.length === 0 ? (
                    ...
                    list.map((emp) => (
                      <TableRow key={emp.id} hover sx={{ cursor: 'pointer' }} onClick={() => handleViewDetail(emp.id)}>
                        ...
                        <TableCell>
                          <Typography variant="body2">{emp.designation?.name || '—'}</Typography>
                        </TableCell>
                        <TableCell>
                          <Chip
                            label={emp.status}
                        ...
                        <TableCell align="right">
                          <Box onClick={(e) => e.stopPropagation()} sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.5 }}>
                            <Tooltip title="View">
                              <IconButton size="small" onClick={() => handleViewDetail(emp.id)}>
                                <ViewIcon fontSize="small" />
                              </IconButton>
                            </Tooltip>
                            <Tooltip title="Edit">
                              <IconButton size="small" color="primary" onClick={() => handleEdit(emp.id)}>
                                <EditIcon fontSize="small" />
                              </IconButton>
                            </Tooltip>
                            <Tooltip title="Delete">
                              <IconButton size="small" color="error" onClick={() => handleDeleteConfirm(emp.id)}>
      ...
      <Dialog open={createOpen} onClose={handleDialogClose} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers>
          <EmployeeForm onSubmit={handleCreateSubmit} saving={saving} />
        </DialogContent>
        <DialogActions>
          <Button onClick={handleDialogClose} disabled={saving}>Cancel</Button>
          <Button variant="contained" type="submit" form="employee-form" disabled={saving}>
            {saving ? <CircularProgress size={20} /> : 'Create Employee'}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={editOpen} onClose={handleDialogClose} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers>
          <EmployeeForm
            ...
            saving={saving}
            initialData={useSelector((state) => state.employees.selectedEmployee)}
            isEdit
          />
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={detailOpen} onClose={handleDialogClose} maxWidth="md" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={() => { handleDialogClose(); handleEdit(selectedId); }}>
            Edit Employee
          </Button>
        </DialogActions>
      </Dialog>
